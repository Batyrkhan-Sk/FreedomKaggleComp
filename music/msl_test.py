...
CTRL = 0.37966          # standing control; its 3-seed ensemble is 0.38023
ENS3 = 0.38023
...
def go(lbl, seeds=(0,1,2), **kw):
    ...
    print(f"  {lbl:<32} mean {v.mean():.5f} sd {v.std(ddof=1):.5f}  "
          f"delta {v.mean()-CTRL:+.5f}   ens3 {ens:.5f} ({ens-ENS3:+.5f})", flush=True)
    return v.mean()

ARMS = [
    ("min_samples_leaf=3000",   dict(min_samples_leaf=3000)),
    ("min_samples_leaf=10000",  dict(min_samples_leaf=10000)),
    ("min_samples_leaf=30000",  dict(min_samples_leaf=30000)),
    ("msl=1000 + l2=10",        dict(min_samples_leaf=1000, l2_regularization=10.0)),
]
want = sys.argv[1:]
for lbl, kw in ARMS:
