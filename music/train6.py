"""Two-stage ranker with user-level AND session-level co-visitation."""
...
from session_covis import session_covis_features
from train4 import load_all

FEATS = BASE_FEATS + ["covis", "covis_rank", "scovis", "scovis_rank"]
...
def featurise(inter, meta, artists, genres, users, cut):
    ...
    C = (
        C.join(covis_features(inter, users, pool, cut), on=["user_id", "item_id"], how="left")
         .join(session_covis_features(inter, users, pool, cut), on=["user_id", "item_id"], how="left")
         .with_columns(pl.col("covis").fill_null(0.0), pl.col("scovis").fill_null(0.0))
    )
    return C.with_columns(
        pl.col("covis").rank("ordinal", descending=True).over("user_id").alias("covis_rank"),
        pl.col("scovis").rank("ordinal", descending=True).over("user_id").alias("scovis_rank"),
    )
...
def main():
    ...
    for cut, hi in WINDOWS:
        ...
        print(f"  {p.height:,} cands, scovis nonzero on {p.filter(pl.col('scovis')>0).height:,}")
    ...
    tot = hit["frac"].sum()
    print(f"\n  ACTIVE-only   {tot/active/TOPK:.5f}   (covis-only was 0.29170)")
    print(f"  projected LB  ~{tot/active/TOPK*1.28:.3f}   (current real 0.37553)")
