...
def covis_features(inter, users, pool_items, cut, window_days=30,
    ...
    # Seeds: each test user's most recent distinct tracks.
    # `age` is in whole DAYS, so a user with 40 plays yesterday has 40 tied rows.
    # sort+unique(keep="first") then picked among those ties by polars row
    # emission order, which is NOT stable across processes -- three separate
    # builds of this function agreed on only ~53% of rows. Dedup via an
    # aggregate (min age per pair) and sort on a TOTAL key instead.
    seeds = (
        recent.filter(pl.col("user_id").is_in(users.implode()))
        .group_by(["user_id", "item_id"])
        .agg(pl.col("age").min().alias("age"))
